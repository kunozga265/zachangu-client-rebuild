    ...
    public function show($code)
    {
        ...
        if (is_object($loan)) {
            ...
            return Inertia::render('Loan/Show', [
                'termsAndConditions'=>$this->termsAndConditions($loan, $loan->physicalAddress,$loan->dueDate),
                'loan' => $loan,
...
//            'termsAndConditions'   => ['required'],
    ...
    public function apply(Request $request,$code){
        ...
        if (is_object($loan)) {
            $loan->update([
                'progress' => 1,
                'termsAndConditions' => $this->termsAndConditions($loan,json_decode($loan->physicalAddress),date('jS F, Y',$loan->dueDate)),
